// 二分：A 页禁用 RemotePlayer，验证 peer_join 冻结点
...
const browser = await puppeteer.launch({
  ...
  headless: 'new', protocolTimeout: 10000,
...
const beats = async (p, tag) => {
  try {
    ...
    console.log(`${tag}: ${b1} -> ${b2} ${b2 > b1 ? 'ALIVE' : 'FROZEN'}`);
  } catch (e) { console.log(`${tag} eval FAIL: ${e.message.slice(0, 60)}`); }
};

const A = await mk('A');
await A.evaluate(() => { window.__NO_REMOTE = true; });   // 禁用远端玩家构建
...
await sleep(1500);
console.log('B:', await B.evaluate(() => window.__DBG.state()));
await beats(A, 'A(无Remote)');
await beats(B, 'B');
await browser.close();
relay.kill();
